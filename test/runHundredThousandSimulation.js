...
const TOTAL_GAMES = parseInt(process.argv[2], 10) || 100000;
const EXPERIMENTS_COUNT = 4;
const EXPERIMENT_GAMES = Math.floor(TOTAL_GAMES / EXPERIMENTS_COUNT);
const numWorkers = Math.min(16, os.cpus().length || 4);

if (isMainThread) {
  console.log("================================================================================");
  console.log("🏆 100.000 MAÇLIK MASTER DENGE VE 250₺ KODES CEZASI SİMÜLASYONU");
  console.log(`📊 Toplam Hedef: ${TOTAL_GAMES.toLocaleString("tr-TR")} Maç | 4 Deney x ${EXPERIMENT_GAMES.toLocaleString("tr-TR")}`);
  console.log("⚡ Deney 1: Grandmaster Usta Oyuncular (Oyun Teorisi Self-Play) [25K]");
  console.log("⚡ Deney 2: Sen (Grandmaster) vs Karma Bot Seviyeleri [25K]");
  console.log("⚡ Deney 3: Botlar Kendi Arasında (İmkansız / Zor / Orta / Kolay) [25K]");
  console.log("⚡ Deney 4: Kıyamet, Kayyum (%0 Kira) & 250₺ Mecburi Tahliye Stres Testi [25K]");
  console.log(`🔥 Donanım: ${numWorkers} Çekirdek AMD Ryzen | İşçi Başına: ${Math.floor(EXPERIMENT_GAMES / numWorkers).toLocaleString("tr-TR")} Maç/Deney`);
  console.log("================================================================================\n");

  async function runExperiment(name, key) {
    return new Promise((resolve) => {
      ...
      const combined = {
        ...
        jailEntries: 0,
        jailExitsDoubles: 0,
        jailEarlyExitsPaid: 0,
        jailForcedExitsNormal: 0, // 65₺
        jailForcedExitsHyper: 0,  // 250₺ (40+ tur)
        jailForcedFineTotal: 0,   // Toplam tahsil edilen mecburi ceza (TL)
        jailZeroRentBlockedCount: 0, // 40+ turda mülk sahibinin kodeste olup 0₺ ödenen kira
        jailStalls: 0,
        threeHouseWins: 0,
        hotelWins: 0,
        railroadWins: 0,
      ...
      for (let i = 0; i < numWorkers; i++) {
        const worker = new Worker(__filename, {
          workerData: { workerId: i, games: gamesPerWorker, mode: key }
        });
        ...
        worker.on("message", (msg) => {
          if (msg.type === "progress") {
            ...
            process.stdout.write(`\r  [${key}] ${totalSimulated.toLocaleString("tr-TR")} / ${EXPERIMENT_GAMES.toLocaleString("tr-TR")} (%${((totalSimulated / EXPERIMENT_GAMES) * 100).toFixed(1)}) | Hız: ${rate.toLocaleString("tr-TR")} maç/s`);
            ...
            combined.minRounds = Math.min(combined.minRounds, r.minRounds);
            combined.maxRounds = Math.max(combined.maxRounds, r.maxRounds);

            combined.jailEntries += r.jailEntries;
            combined.jailExitsDoubles += r.jailExitsDoubles;
            combined.jailEarlyExitsPaid += r.jailEarlyExitsPaid;
            combined.jailForcedExitsNormal += r.jailForcedExitsNormal;
            combined.jailForcedExitsHyper += r.jailForcedExitsHyper;
            combined.jailForcedFineTotal += r.jailForcedFineTotal;
            ...
            combined.wealthTaxCollectedSum += r.wealthTaxCollectedSum;
            combined.wealthTaxInstances += r.wealthTaxInstances;
            combined.wealthTaxShieldSaves += r.wealthTaxShieldSaves;
            combined.wealthTaxShieldSavedTL += r.wealthTaxShieldSavedTL;
  ...
  async function runAll() {
    ...
    const exp4 = await runExperiment("DENEY 4: KIYAMET, KAYYUM & 250₺ KODES CEZASI", "CRISIS_STRESS");

    const totalSec = ((Date.now() - overallStart) / 1000).toFixed(2);
    console.log("================================================================================");
    console.log(`🎉 TÜM 100.000 MAÇ ${totalSec} SANİYEDE TAMAMLANDI!`);
    console.log("================================================================================\n");

    const finalReport = {
      timestamp: new Date().toISOString(),
      totalDurationSeconds: totalSec,
      totalSimulatedGames: TOTAL_GAMES,
      experiments: { exp1, exp2, exp3, exp4 }
    };

    fs.writeFileSync(
      new URL("./hundredThousandResultsFinal.json", import.meta.url),
      JSON.stringify(finalReport, null, 2),
      "utf-8"
    );
    console.log("📁 Sonuçlar test/hundredThousandResultsFinal.json dosyasına kaydedildi.\n");
  }
  ...
  const res = {
    ...
    jailExitsDoubles: 0,
    jailEarlyExitsPaid: 0,
    jailForcedExitsNormal: 0,
    jailForcedExitsHyper: 0,
    jailForcedFineTotal: 0,
  ...
  const reportBatch = Math.max(250, Math.floor(games / 25));
  let unrecorded = 0;
  ...
  for (let g = 0; g < games; g++) {
    ...
    while (game.status === "playing" && rounds < MAX_ROUNDS && turnCount < 400) {
      ...
      if (active.money < 0) {
        ...
        // 5. Acil Banka Kredisi (40. turdan sonra iflas engeli)
        if (active.money < 0) {
          if (currentRound >= 40) {
      ...
      // Kodes Kararı (250₺ Mecburi Tahliye Cezası & %0 Kayyum Blokesi Telemetrisi)
      if (active.inJail) {
        res.jailEntries++;
      ...
      if (game.phase === "WAITING_ROLL") {
        ...
        if (wasInJail) {
          if (!active.inJail) {
            if (prevTurns >= 2) {
              // 3. Tur sonu mecburi tahliye
              if (currentRound >= 40) {
                res.jailForcedExitsHyper++;
                res.jailForcedFineTotal += 250;
              } else {
                res.jailForcedExitsNormal++;
                res.jailForcedFineTotal += 65;
              }
            } else {
              // Çift atıp beraat etti
              res.jailExitsDoubles++;
            }
          }
        }
      }
    ...
    if (game.winner) {
      ...
      res.minRounds = Math.min(res.minRounds, endRound);
      res.maxRounds = Math.max(res.maxRounds, endRound);
      ...
      if (game.stats) {
        res.wealthTaxCollectedSum += (game.stats.wealthTaxCollectedSum || 0);
        res.wealthTaxInstances += (game.stats.wealthTaxInstances || 0);
        res.wealthTaxShieldSaves += (game.stats.wealthTaxShieldSaves || 0);
        res.wealthTaxShieldSavedTL += (game.stats.wealthTaxShieldSavedTL || 0);
      }
    } else {
      res.turnTimeouts++;
    }
