...
if (isMainThread) {
  console.log(`================================================================================`);
  console.log(`🏆 400.000 MAÇLIK DÖRT BÜYÜK KARŞILAŞTIRMA VE SİSTEM ÇÖKÜŞÜ SİMÜLASYONU`);
  ...
  console.log(`⚡ Deney 4: 100.000 Maç Ekstrem Kriz & Hiperenflasyon Dayanıklılık Testi`);
      ...
      for (let i = 0; i < numWorkers; i++) {
        ...
        worker.on("message", (msg) => {
          if (msg.type === "progress") {
            ...
            combined.freeParkingJackpotsClaimed += r.freeParkingJackpotsClaimed;
  ...
  async function runAll() {
    ...
    const exp4 = await runExperiment("DENEY 4: HİPERENFLASYON & AFET VERGİSİ STRES TESTİ", "CRISIS_STRESS");

    const totalSec = ((Date.now() - overallStart) / 1000).toFixed(2);
    console.log(`================================================================================`);
    console.log(`🎉 TÜM 400.000 MAÇ ${totalSec} SANİYEDE TAMAMLANDI!`);
    console.log(`================================================================================\n`);

    const finalReport = {
      timestamp: new Date().toISOString(),
      totalDurationSeconds: totalSec,
      totalSimulatedGames: EXPERIMENT_GAMES * 4,
      experiments: { exp1, exp2, exp3, exp4 }
    };

    fs.writeFileSync(
      new URL("./fourHundredThousandResults.json", import.meta.url),
      JSON.stringify(finalReport, null, 2),
      "utf-8"
    );
    console.log("📁 Sonuçlar test/fourHundredThousandResults.json dosyasına kaydedildi.\n");
  }
  ...
  for (let g = 0; g < games; g++) {
    ...
    if (mode === "SELF_PLAY") {
      ...
      // 4 Oyuncu: 2 Grandmaster, 2 İmkansız Bot - Kriz dayanıklılığı
      const stressRoles = [
    ...
    while (game.status === "playing" && rounds < MAX_ROUNDS && turnCount < 400) {
      ...
      if (active.inJail) {
        ...
        if (strat === "GRANDMASTER" || strat === "AUCTION_SHARK" || strat === "BOT_IMKANSIZ") {
          ...
          if (active.jailCards > 0) game.useJailCard(active.id);
          else if (active.money >= jailFine + (strat === "BOT_ZOR" ? 60 : 120)) game.payJailFine(active.id);
        }
      }
      ...
      // İnşaat Fazı (Three-House Optimal Rule & Otel İndirimi)
      if (game.phase === "TURN_ACTIONS" && !active.isBankrupt) {
        ...
        for (const groupName in COLOR_GROUPS) {
          ...
          if (ownsAll) {
            for (const tId of groupTiles) {
              ...
              // 3 Ev Kuralı: 3 eve kadar öncelikli inşa, otel için %15 indirimle veya bol nakit varsa yükselt
        ...
        // İpotek Kaldırma Stratejisi (Nakit bolsa ve %50 faiz caydırmıyorsa)
        if (active.money > 500) {
          for (const tId in game.properties) {
            const prop = game.properties[tId];
            if (prop.ownerId === active.id && prop.mortgaged) {
