...
function PlayerPanelBase({ gameState, myPlayerId, onOpenTrade, onTileClick, onRemoveBot, onSetBotDifficulty, onKickPlayer, onOpenDeeds, isDarkMode }) {
  ...
  return (
    ...
    <div className="cardstock-panel rounded-3xl p-3 sm:p-4 flex flex-col gap-2.5 text-slate-900 dark:text-slate-100 flex-shrink-0">
      <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
        <h2 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest flex items-center gap-1.5 font-space">
          <Users className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
          <span>Oyuncu Durumları</span>
        </h2>
        <div className="flex items-center gap-1.5">
          {/* Tapu Senetleri Butonu (Tıklanınca modal açar) */}
          {onOpenDeeds && (
            <button
              onClick={onOpenDeeds}
              className="px-2 py-0.5 rounded-lg bg-sky-500/15 hover:bg-sky-500/25 border border-sky-400/40 text-sky-700 dark:text-sky-300 transition cursor-pointer flex items-center gap-1 text-[10px] font-bold font-space shadow-xs active:scale-95"
              title="Tapu Senetleri Portföyünü Gör"
            >
          ...
          <button
            ...
            className="p-1 sm:px-1.5 py-0.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 text-slate-700 hover:text-slate-950 dark:text-slate-200 dark:hover:text-white transition cursor-pointer flex items-center gap-1 text-[10px] font-bold font-jetbrains shadow-xs active:scale-95"
            title={isCompact ? 'Geniş Görünüme Geç' : 'Kompakt Görünüme Geç'}
          >
